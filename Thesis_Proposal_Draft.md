...
| 数据 | 来源 | 粒度 | 时间范围 |
|------|------|------|---------|
| 碳强度 CI (gCO₂eq/kWh) | ElectricityMaps API | Hourly | 2024–2025 |
| 可再生能源占比 RF (%) | ElectricityMaps API | Hourly | 2024–2025 |
| 电价（约束参数）| PJM / NYISO / ENTSO-E / EMC | Hourly | 2024–2025 |
...
```
历史 CI/RF 数据（ElectricityMaps，2024–2025，17,544 小时/地区）
          ↓
Unified Joint LP（Gurobi）
  决策变量 x_{r,t}：同时覆盖时间 + 空间维度
          ↓
Evaluation（Backtesting vs. Baselines）
  + 事后分解：时间 vs. 空间各自的边际碳节省
```
...
(C4) 电价预算：  Σ_t x_{r,t}·P_r(t) ≤ Budget_r  ∀r      [建模选择；见正文说明]
(C5) 非负：      x_{r,t} ≥ 0
```
...
| C4 电价预算约束 | Hao, Liu & Deng (2024), Frontiers in Energy Research, doi:10.3389/fenrg.2024.1344837 |
| 跨地区时空联合调度框架 | Riepin, Brown & Zavala (2025), Advances in Applied Energy; Bashir et al. (2024), arXiv:2512.08725 |
| RF 加权项 (1−α·RF) | 本文创新，受 Liu et al. (2012)（renewable-aware调度）和 Riepin et al. (2025)（CFE fraction信号）启发 |
...
2. **时空联合优化**：统一 LP 同时优化时间与空间维度，事后分解量化各维度的边际贡献
